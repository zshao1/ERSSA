...
Currently, the most up-to-date version can be found on [Github](https://github.com/zshao1/ERSSA). The package can be installed from Github with the following commands in the R console:

```{r, eval=FALSE}
install.packages("devtools") #install devtools package
library(devtools) #load devtools
install_github("zshao1/ERSSA") #install from Github
```
...
Since a random process is used to generate the combinations, each individual `erssa` run will generate an unique set of sample combinations as long as all of the unique combinations have not been exhausted as set by the `comb_gen_repeat` argument. However, a random seed can be fixed for reproducible results by setting the `comb_gen_seed` argument to a number. For example, We can reproduce the number of DE genes plot in section 3.1 using `comb_gen_seed=1`. Additionally, it is worth noting that we found it useful to run several random seeds to confirm the overall conclusions are consistent across individual runs.
...
ssa = erssa(count_table = count_table, condition_table = condition_table,
            DE_ctrl_cond = 'Nigerian', comb_gen_seed = 1,
            , num_workers = 8, comb_gen_repeat = 50)
```
...
# Start ERSSA
ssa = erssa(count_table = count_table, condition_table = condition_table,
            DE_ctrl_cond = 'Control', comb_gen_seed = 1,
