...
#' @param cutoff_stat The cutoff in FDR for DE consideration. Genes with lower FDR pass the cutoff. Default = 0.05.
...
erssa_edger = function(count_table.filtered=NULL, combinations=NULL, condition_table=NULL, control=NULL, cutoff_stat = 0.05, cutoff_Abs_logFC = 1, save_table=FALSE, path='.'){

  #check all required arguments supplied
  if (is.null(count_table.filtered)){
    stop('Missing required count_table.filtered argument in erssa_edger function')
  ...
  cat(paste0('Start classic edgeR Exact Test with FDR cutoff = ',cutoff_stat, ', Abs(logFC) cutoff = ', cutoff_Abs_logFC,'\n'))
  ...
  for (rep_level in names(combinations)){
    ...
    for (index in seq(1, length(comb_rl))){
      ...
      DE_et_cutoff = DE_et[which(DE_et$FDR < cutoff_stat & abs(DE_et$logFC) > cutoff_Abs_logFC),]
      ...
      if (save_table==TRUE){
        utils::write.csv(DE_et, file = file.path(folder_path,paste0('ERSSA_edgeR_',rep_level,'_comb',index,'.csv')))
      }
