...
ggplot2_TPR_FPRPlot = function(deg=NULL, count_table.filtered=NULL, path='.'){
  ...
  if (is.null(deg)){
    ...
    stop('More than one data type detected in count table, please make
         sure count table contains only numbers and that the list of gene
         names is the data.frame index')
  }
  ...
  gg = ggplot2::ggplot(TPR_FPR.dataframe, ggplot2::aes(
    FPR, TPR, colour=replicate)) +
