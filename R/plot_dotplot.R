...
#' @param path Path to which the plot will be saved. Default to current working directory.
#'
...
ggplot2_dotplot = function(deg=NULL, path='.'){
  ...
  gg = ggplot2::ggplot(deg_df, ggplot2::aes(rep_level, num_DEG)) +
    ...
    ggplot2::geom_dotplot(binaxis='y', stackdir='center',dotsize=2, binwidth = 30) +
    ggplot2::theme_bw() +
    ggplot2::geom_hline(ggplot2::aes(yintercept=full_num_DEG, color = "Full dataset"),
               size=0.75, linetype="dashed", show.legend = TRUE) +
    ggplot2::stat_summary(ggplot2::aes(rep_level, num_DEG, colour="Mean"), group=1, fun.y='mean',
                 geom='line', size=1) +
    ...
    ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(linetype = c("dashed", "solid"))))
